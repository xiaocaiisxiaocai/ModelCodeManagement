    ...
    public class OrganizationService : IOrganizationService
    {
        ...
        public async Task<ApiResponse<OrganizationDto>> CreateAsync(CreateOrganizationDto dto, int? createdBy = null)
        {
            try
            {
                ...
                {
                    _logger.LogWarning("创建组织架构失败 - 编码已存在: {Code}", dto.Code);
                    return ApiResponse<OrganizationDto>.ErrorResult($"组织编码 '{dto.Code}' 已存在");
                }
                ...
                try
                {
                    ...
                    await _auditLogService.LogActionAsync("CreateOrganization", 
                        $"创建组织架构: {dto.Name} ({dto.Code})", "Organization", entity.Id);
                    
                    _logger.LogInformation("创建组织架构成功 - Code: {Code}, Name: {Name}, Id: {Id}", 
            ...
            {
                _logger.LogError(ex, "创建组织架构失败 - Code: {Code}, Name: {Name}", dto.Code, dto.Name);
        ...
        public async Task<ApiResponse<OrganizationDto>> UpdateAsync(int id, UpdateOrganizationDto dto, int? updatedBy = null)
        {
            try
            {
                ...
                {
                    _logger.LogWarning("更新组织架构失败 - 编码已存在: {Code}", dto.Code);
                    return ApiResponse<OrganizationDto>.ErrorResult($"组织编码 '{dto.Code}' 已存在");
                }
                ...
                await _auditLogService.LogActionAsync("UpdateOrganization", 
                    $"更新组织架构: {dto.Name} ({dto.Code})", "Organization", entity.Id);
        ...
        public async Task<ApiResponse> DeleteAsync(int id)
        {
            try
            {
                ...
                await _auditLogService.LogActionAsync("DeleteOrganization", 
                    $"删除组织架构: {entity.Name} ({entity.Code})", "Organization", entity.Id);
        ...
        private async Task<OrganizationDto> MapToDto(Organization entity)
        {
            ...
            // 获取用户数量
            dto.UserCount = await _context.Users
                .Where(x => x.OrganizationId == entity.Id)
                .CountAsync();
