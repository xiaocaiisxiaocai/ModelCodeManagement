...
import type { CodeClassificationDto as CodeClassification } from '../services/apiService';
...
const CodeClassificationPage: React.FC = () => {
  ...
  const getCodeClassifications = useCallback(() => {
    ...
    return unifiedServices.codeClassification.getCodeClassificationsByModelType(modelType);
  }, [modelType]);
  ...
  const handleSaveCode = async (code: CodeClassification) => {
    ...
    if (drawerMode === 'create') {
      const result = await handleResponse(
        () => unifiedServices.codeClassification.addCodeClassification(code, productType || ''),
        (data) => {
          showSuccess(`成功创建代码分类: ${data.code}`);
          success = true;
        },
        (errorMsg) => {
      ...
      const result = await handleResponse(
        () => unifiedServices.codeClassification.updateCodeClassification(selectedCode.id.toString(), code),
        (data) => {
          showSuccess(`成功更新代码分类: ${data.code}`);
          success = true;
        },
        (errorMsg) => {
  ...
  const handleDeleteCodeConfirm = async (code: string) => {
    // 根据code找到对应的ID
    const codeClassification = codeClassifications.find(cc => cc.code === code);
    ...
    const result = await handleResponse(
      () => unifiedServices.codeClassification.deleteCodeClassification(codeClassification.id.toString()),
      () => {
  ...
  const sortedCodeClassifications = useMemo(() => {
    ...
    return [...codeClassifications].sort((a, b) => {
