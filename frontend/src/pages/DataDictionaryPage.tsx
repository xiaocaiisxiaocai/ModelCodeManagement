...
import type { Customer, Factory, ProductNameDict, OccupancyTypeDict } from '../mock/interfaces';
...
const DataDictionaryPage: React.FC = () => {
  ...
  const [dataDictionary, setDataDictionary] = useState({
    ...
    productNames: [] as ProductNameDict[],
    occupancyTypes: [] as OccupancyTypeDict[]
  });
  ...
  const loadDataDictionary = async () => {
    ...
    try {
      const [customersRes, factoriesRes, productNamesRes, occupancyTypesRes] = await Promise.all([
        unifiedServices.dataDictionary.getCustomers(),
        unifiedServices.dataDictionary.getFactories(),
        unifiedServices.dataDictionary.getProductNames(),
        unifiedServices.dataDictionary.getOccupancyTypes()
      ]);

      // 确保所有响应都成功
      if (customersRes.success && factoriesRes.success && productNamesRes.success && 
          occupancyTypesRes.success) {
        // 确保数据是数组格式
        const customers = Array.isArray(customersRes.data) ? customersRes.data : [];
        const factories = Array.isArray(factoriesRes.data) ? factoriesRes.data : [];
        const productNames = Array.isArray(productNamesRes.data) ? productNamesRes.data : [];
        const occupancyTypes = Array.isArray(occupancyTypesRes.data) ? occupancyTypesRes.data : [];
  ...
  const getFilteredData = (data: any[], searchFields: string[]) => {
    ...
    if (!Array.isArray(data)) {
      console.warn('getFilteredData: data is not an array:', data);
      return [];
    }
  ...
  const getFilteredFactories = () => {
    ...
    if (!Array.isArray(factories)) {
      console.warn('getFilteredFactories: factories is not an array:', factories);
      return [];
    }
    
    if (selectedCustomer) {
      factories = factories.filter(factory => factory.customerId === selectedCustomer);
    }
  ...
  const getPaginatedData = (data: any[]) => {
    // 确保 data 是数组
    if (!Array.isArray(data)) {
      console.warn('getPaginatedData: data is not an array:', data);
      return [];
    }
  ...
  const getTotalPages = (data: any[]) => {
    // 确保 data 是数组
    if (!Array.isArray(data)) {
      console.warn('getTotalPages: data is not an array:', data);
      return 1;
    }
  ...
  const confirmDelete = async () => {
    ...
    setLoading(true);
    try {
      const response = await unifiedServices.dataDictionary.delete(parseInt(deleteTargetId));
          ...
          case 'customers':
            return [
              ...
              <div key="count" className="text-left font-mono text-sm font-medium text-blue-600">
                {dataDictionary.factories.filter(f => f.customerId === item.id).length}
              </div>,
          ...
          case 'factories':
            return [
              ...
              <span key="customer" className="text-sm text-gray-600">
                {dataDictionary.customers.find(c => c.id === item.customerId)?.name || '-'}
              </span>,
              <div key="actions" className="flex gap-1 min-w-max">
                <Button 
                  variant="secondary" 
