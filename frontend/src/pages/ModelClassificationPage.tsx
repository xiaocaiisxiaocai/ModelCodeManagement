...
import type { ModelClassification } from '../mock/interfaces';
...
const ModelClassificationPage: React.FC = () => {
  ...
  const getModelClassifications = useCallback(() => {
    ...
    return unifiedServices.modelClassification.getModelClassificationsByProductType(productType);
  }, [productType]);
  ...
  const handleSaveModel = async (model: ModelClassification) => {
    ...
    if (drawerMode === 'create') {
      const result = await handleResponse(
        () => unifiedServices.modelClassification.addModelClassification(model),
        (data) => {
          showSuccess(`成功创建机型分类: ${data.type}`);
          success = true;
        },
        (errorMsg) => {
