...
import type { ProductType } from '../mock/interfaces';
...
const ProductTypePage: React.FC = () => {
  ...
  // 使用新的 useApi Hook 来获取数据
  const getProductTypes = useCallback(() => unifiedServices.productType.getAllProductTypes(), []);
  const { data: productTypes, loading: dataLoading, error: dataError, refetch: loadProductTypes } = useApi(getProductTypes);
  ...
  const handleSaveProduct = async (product: Omit<ProductType, 'id'>) => {
    ...
    if (drawerMode === 'create') {
      const result = await handleResponse(
        ...
        (data) => {
          showSuccess(`成功创建产品类型: ${data.code}`);
          success = true;
        },
        (errorMsg) => {
      ...
      const result = await handleResponse(
        () => unifiedServices.productType.updateProductType(selectedProduct.id, product as ProductType),
        (data) => {
          showSuccess(`成功更新产品类型: ${data.code}`);
          success = true;
        },
        (errorMsg) => {
