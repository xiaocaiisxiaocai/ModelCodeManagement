...
import type { Permission, CreatePermissionRequest, UpdatePermissionRequest } from '../../services/rbacApiService';
...
const PermissionManagementPage: React.FC = () => {
  ...
  const loadPermissions = async () => {
    setLoading(true);
    try {
      const response = await rbacApiService.getPermissions({ pageSize: 100 });
  ...
  const handleCreatePermission = async () => {
    ...
    setLoading(true);
    try {
      const response = await rbacApiService.createPermission({
  ...
  const handleEditPermission = async () => {
    ...
    setLoading(true);
    try {
      const response = await rbacApiService.updatePermission(editingPermission.id, {
        code: editForm.code,
        name: editForm.name,
        type: editForm.type,
  ...
  const handleDeletePermission = async (permissionId: number) => {
    if (!confirm('确定要删除这个权限吗？删除后相关角色的权限配置也会受到影响。')) return;

    setLoading(true);
    try {
      const response = await rbacApiService.deletePermission(permissionId);
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center">
                            <span className="text-xs text-gray-500 w-16">级别:</span>
          ...
          <Card className="hidden lg:block">
            <CardBody className="p-0">
              <div className="overflow-x-auto">
                <Table
                  headers={['权限编码', '权限名称', '权限类型', '权限级别', '资源', '操作']}
