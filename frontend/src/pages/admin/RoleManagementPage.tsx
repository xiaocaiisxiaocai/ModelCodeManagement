...
import { Button, Card, CardBody, Input, Modal, ModalFooter, Table, Badge, useToast } from '../../components/ui';
import PermissionGate from '../../components/auth/PermissionGate';
import { rbacApiService } from '../../services/rbacApiService';
import type { Role, Permission } from '../../services/rbacApiService';

// Role 和 Permission 接口已经从 rbacApiService 导入
...
const RoleManagementPage: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [rolesResponse, permissionsResponse] = await Promise.all([
        rbacApiService.getRoles({ pageSize: 100 }),
        rbacApiService.getPermissions({ pageSize: 100 })
      ]);
  ...
  const handleCreateRole = async () => {
    ...
    try {
      // 使用后端API创建角色
      const response = await rbacApiService.createRole({
      ...
      if (createForm.permissions.length > 0) {
        await rbacApiService.assignRolePermissions(newRole.id, createForm.permissions);
      }
  ...
  const handleEditRole = async () => {
    ...
    try {
      // 使用后端API更新角色
      const response = await rbacApiService.updateRole(editingRole.id, {
        code: editForm.code,
        name: editForm.name
      });
      ...
      if (editForm.permissions.length > 0) {
        await rbacApiService.assignRolePermissions(editingRole.id, editForm.permissions);
      }
  ...
  const handleDeleteRole = async (roleId: number) => {
    ...
    try {
      // 使用后端API删除角色
      const response = await rbacApiService.deleteRole(roleId);
  ...
  const openEditModal = async (role: Role) => {
    ...
    // 获取角色的当前权限
    try {
      const rolePermissions = await rbacApiService.getRolePermissions(role.id);
  ...
  const openPermissionModal = async (role: Role) => {
    ...
    try {
      ...
      if (rolePermissions.success && rolePermissions.data) {
        const enrichedRole = {
          ...role,
          permissions: rolePermissions.data.map(p => p.id)
        };
